...
exports.validateAppName = function (name) {
  ...
  if(name.match(/[\/@\s\+%:]/)) {
    return 'Application name cannot contain special characters (/@+%: ): ' + name;
  }
  ...
  if(name !== encodeURIComponent(name)) {
    return 'Application name cannot contain special characters escaped by encodeURIComponent: ' + name;
  }
  return true;
};
